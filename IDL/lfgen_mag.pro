...
function lfgen_mag,lumin,lumerrin,XBIN=XBIN,YBIN=YBIN,ERR=ERR, $
  ...
  lfbinsize=( maxall-minall )/nbin

  for m=0, nbin-1 do begin  
    ;###################################equal sized bins
    if (keyword_set(eqbin) eq 1) then begin 
      
      minbin=double(minall+(m*lfbinsize))
      maxbin=double(minbin+lfbinsize) ;double precesion required for # > 1e36-ish
      indbin=where(lumsort ge minbin and lumsort le maxbin)  ;locate objects inside the equal sized bins
      ;lummax=10.^maxbin
      ;lummin=10.^minbin
      bincen[m]=( maxbin+minbin )/2.
    ...
    ;###################################equal number bins
      ...
      maxbin=double(max(lumbin))
      minbin=min(lumbin)
      bincen[m]=( maxbin+minbin )/2.
      ;bincen[m]=median(alog10(lumsort[indbin]))

      if (numbin eq 1 or maxbin eq minbin) then begin
        dndl[m]=numbin/(maxbin*0.9)
      endif else begin
        dndl[m]=numbin/(maxbin - minbin)
        if finite(dndl[m]) eq 0 then stop
      endelse
  ...
  if (n_elements(nbinhist) ne 0) then begin
    y=histogram(lum,nbin=nbinhist,locations=x)  
  endif else begin
    y=histogram(lum,bin=binsize,locations=x)
  endelse
  ...
  ;no good fit
  if (fit[0] eq 0. and fit[1] eq 0.) then begin
    perror=[0./0.,0./0.]
    fit=[0./0.,0./0.]
  endif
